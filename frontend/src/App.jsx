    ...
    const fetchPrediction = useCallback(async () => {
        ...
        try {
            const res = await fetch(
                `http://localhost:8000/api/v1/predict/${selectedModel}`,
                { method: "POST" },
            );
            ...
            <header className="h-12 border-b border-[#30363d] bg-[#0d1117] flex items-center px-4 justify-between shrink-0">
                ...
                <div className="flex items-center space-x-4">
                    <div className="text-right">
                        <p className="label-muted text-[8px]! leading-none mb-0.5">
                            EST. LATENCY
                        </p>
                        <p className="value-mono text-[10px] text-[#3fb950] leading-none">
                            12ms
                        </p>
                    </div>
                    <div className="h-6 w-px bg-[#30363d]"></div>
            ...
            <main className="flex-1 min-h-0">
                {activeTab === "terminal" && (
                    <div className="h-full grid grid-cols-12">
                            ...
                            <div className="h-8 border-b border-[#30363d] bg-[#0d1117] flex items-center px-4 space-x-6">
                                ...
                                <div className="flex space-x-4">
                                    <div className="flex space-x-1.5">
                                        ...
                                        <span className="value-mono text-[9px] text-[#f85149]">
                                            {liveData?.chart?.candles
                                                ?.at(-1)
                                                ?.low?.toFixed(5)}
                                        </span>
                                    </div>
                                    <div className="flex space-x-1.5">
                                        ...
                                        <span className="value-mono text-[9px] text-[#3fb950]">
                                            {liveData?.chart?.candles
                                                ?.at(-1)
                                                ?.high?.toFixed(5)}
                                        </span>
                        ...
                        <div className="col-span-3 border-l border-[#30363d] flex flex-col min-h-0 overflow-hidden bg-[#06090f]">
                            <OrderTicket
                                liveData={liveData}
                    ...
                    <div className="h-full p-4 overflow-y-auto">
                        <div className="max-w-6xl mx-auto grid grid-cols-2 gap-6 h-[calc(100vh-100px)]">
                            ...
                            <div className="flex flex-col gap-6">
                                <ModelConfigWidget
                                    config={modelDetails?.config}
                                />
                                <ValidationStatsWidget
                                    stats={modelDetails?.stats}
                                />
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex space-x-4 items-center">
                    ...
                    <div className="flex items-center space-x-2 text-[9px] font-bold text-[#8b949e]">
                        ...
                        <span className="text-white uppercase">
                            {modelDetails?.config?.model?.type || "TRANSFORMER"}
                        </span>
                    </div>
                </div>
