...
def execute_market_order(
    symbol: str, lot_size: float, direction: str, atr: float, multiplier: float
):
    """
    ...
    if not terminal_info.trade_allowed:
        return {
            "status": "error",
            "message": "MT5 Error: 'Algo Trading' button is disabled in the terminal toolbar.",
        }
    ...
    if tick is None:
        return {
            "status": "error",
            "message": f"Could not get tick for {symbol}. Check symbol name.",
        }
    ...
    # Auto-Detect Filling Mode (Fixes broker-specific crashes)
    ...
    if result is None:
        return {
            "status": "error",
            "message": "MT5 returned None. Request might be malformed.",
        }
