...
def run_optimization_study(
    config_path: str, n_trials: int = 50, metric: str = "profit"
):
    ...
    if "train_split_pct" in config.get("data", {}):
        ...
        val_df_full = df[
            (df.index >= val_split_date) & (df.index < test_split_date)
        ].copy()

    if len(train_df) == 0 or len(val_df_full) == 0:
        raise ValueError(
            "Train or Validation set is empty. Check your split dates or percentages."
        )
    ...
    if metric == "profit":
        ...
        lookback = model_params.get(
            "lookback", config.get("data", {}).get("lookback", 60)
        )
        ...
        print("--- Pre-training Complete. Starting Strategy Search ---\n")
    ...
    def objective(trial):
        ...
        # Hyperparameters for Data (STRICTLY LOCKED)
        import re
        ...
        if metric == "profit":
            ...
            if model_type == "CNN-LSTM":
                model_params["cnn_filters_1"] = trial.suggest_categorical(
                    "cnn_filters_1", [16, 32, 64]
                )
                model_params["cnn_filters_2"] = trial.suggest_categorical(
                    "cnn_filters_2", [32, 64, 128]
                )
                model_params["lstm_units"] = trial.suggest_categorical(
                    "lstm_units", [32, 50, 100]
                )
                model_params["kernel_size"] = trial.suggest_int("kernel_size", 2, 5)
                model_params["dropout"] = trial.suggest_float(
                    "dropout", 0.1, 0.4, step=0.1
                )
                model_params["weight_decay"] = trial.suggest_float(
                ...
                model_params["lookback"] = trial.suggest_categorical(
                    "lookback", [60, 120, 240]
                )

            # ALIGN DATA & TRAIN (Stage 1 only)
            ...
            if isinstance(model_wrapper, PyTorchBaseModel):
                ...
                train_loader, val_loader, _ = generator.prepare_loaders(
                    train_df.loc[y_series_trial.index],
                    val_df_full,
                    train_df.loc[y_series_trial.index],
                    target_col=target_col,
                )
                best_val_loss = model_wrapper.train_model(
        ...
        # Hyperparameters for Strategy
        exit_range = search_space.get("exit_atr_multiplier", [1.0, 5.0, 0.5])
        exit_atr_multiplier = trial.suggest_float(
            "exit_atr_multiplier",
            exit_range[0],
            exit_range[1],
            step=exit_range[2] if len(exit_range) > 2 else None,
        )
        ...
        conf_threshold = trial.suggest_float(
            ...
            conf_range[0],
            conf_range[1],
            step=conf_range[2] if len(conf_range) > 2 else None,
        )
        ...
        # Maximize Profit Factor, but penalize bad Sharpe Ratios
    ...
    if "horizon" in trial.params:
        ...
        if "Nguyen" in base_target:
            ...
            best_m = trial.params.get(
                "label_atr_multiplier", config["data"].get("atr_multipliers", [2.0])[0]
            )
    ...
    if metric == "loss":
        if model_type == "RandomForest":
            optimized_config["model"]["params"]["n_estimators"] = trial.params[
                "n_estimators"
            ]
            ...
            optimized_config["model"]["params"]["num_layers"] = trial.params[
                "num_layers"
            ]
            optimized_config["model"]["params"]["dropout"] = trial.params["dropout"]
            optimized_config["model"]["params"]["learning_rate"] = trial.params[
                "learning_rate"
            ]
            optimized_config["model"]["params"]["lookback"] = trial.params["lookback"]
        elif model_type == "LSTM":
            optimized_config["model"]["params"]["hidden_dim"] = trial.params[
                "hidden_dim"
            ]
            optimized_config["model"]["params"]["num_layers"] = trial.params[
                "num_layers"
            ...
            optimized_config["model"]["params"]["cnn_filters_1"] = trial.params[
                "cnn_filters_1"
            ]
            optimized_config["model"]["params"]["cnn_filters_2"] = trial.params[
                "cnn_filters_2"
            ]
            optimized_config["model"]["params"]["lstm_units"] = trial.params[
                "lstm_units"
            ]
            optimized_config["model"]["params"]["kernel_size"] = trial.params[
                "kernel_size"
            ]
            optimized_config["model"]["params"]["dropout"] = trial.params["dropout"]
            optimized_config["model"]["params"]["weight_decay"] = trial.params[
                "weight_decay"
            ]
            optimized_config["model"]["params"]["learning_rate"] = trial.params[
                "learning_rate"
    ...
    if metric == "profit":
        optimized_config["backtest"]["atr_multiplier"] = trial.params[
            "exit_atr_multiplier"
        ]
