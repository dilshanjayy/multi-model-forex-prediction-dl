...
def score_headlines(titles, texts):
    ...
    inputs = tokenizer(
        combined, padding=True, truncation=True, max_length=512, return_tensors="pt"
    ).to(device)
...
def aggregate_to_h1(df, output_path):
    ...
    h1_sentiment = (
        df[["sent_pos", "sent_neg", "sent_neu", "sentiment_score"]].resample("H").mean()
    )
...
if __name__ == "__main__":
    ...
    print(
        "You can now download this file and aggregate it to any timeframe (M15, H1, etc.) locally."
    )
