...
class PyTorchBaseModel(BaseModel, nn.Module):
    ...
    def train(self, mode: bool = True, X_train=None, y_train=None):
        """
        Satisfies BaseModel's abstract train method while preserving nn.Module.train().
        PyTorch models should use train_model() with DataLoaders instead.
        """
        if X_train is not None or y_train is not None:
            raise NotImplementedError("PyTorch models use train_model(train_loader, val_loader)")
    ...
    def train_model(self, train_loader, val_loader):
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch + 1) % 5 == 0 or epoch == 0:
                print(f"Epoch {epoch+1}/{self.epochs} | Train Loss: {train_loss/len(train_loader):.4f} | Val Loss: {avg_val:.4f}")
    ...
    def _batch_inference(self, windows: torch.Tensor, batch_size: int = 512) -> torch.Tensor:
        self.eval()
        outputs = []
        with torch.no_grad():
