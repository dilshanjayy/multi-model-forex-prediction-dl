...
def get_global_stats(
    ...
    try:
        ...
        winning_trades = (
            db.query(func.count(models.Trade.id)).filter(models.Trade.pnl > 0).scalar()
            or 0
        )
        ...
        most_used_model = (
            db.query(
                models.Trade.model_used, func.count(models.Trade.id).label("count")
            )
...
async def upload_model(
    file: UploadFile = File(...), admin: models.User = Depends(get_current_admin_user)
):
    ...
    try:
        ...
        if os.path.exists(extract_path):
            raise HTTPException(
                status_code=400, detail=f"A model named {model_name} already exists."
            )
...
def reload_models(admin: models.User = Depends(get_current_admin_user)):
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Model cache successfully hot-reloaded.",
        }
    except Exception as e:
        import traceback
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Failed to reload models: {str(e)}"
        )
